<?php
class ControllerInformationThemeBlog extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('information/themeblog');
		$this->load->model('mytheme/themeblog');
		$this->load->model('tool/image');

		if (!isset($this->request->get['themeblog_id'])) {
			$this->response->redirect($this->url->link('information/themeblog/blogs'));
		}

		$language = $this->model_mytheme_themeblog->getLanguageByCode($this->session->data['language']);

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_mytheme_themeblog->addComment($this->request->get['themeblog_id'], $this->request->post, $language['language_id']);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('information/themeblog', '', true)
		);

		$data['breadcrumbs'][] = array(
			'text' => $blog_info['title'],
			'href' => $this->url->link('information/themeblog', 'themeblog_id=' . $this->request->get['themeblog_id'], true)
		);
		...
		$module = $this->model_mytheme_themeblog->getModule($blog_info['module_id']);
		...
		$blog_comments = $this->model_mytheme_themeblog->getBlogComments($this->request->get['themeblog_id'], $language['language_id']);
		...
		$data['action'] = $this->url->link('information/themeblog', 'themeblog_id=' . $this->request->get['themeblog_id'], true);
		...
		if (!$this->customer->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('information/themeblog', 'themeblog_id=' . $this->request->get['themeblog_id'], true);
		}
		...
		$this->response->setOutput($this->load->view('information/themeblog', $data));
		}
	...
	public function blogs() {
		...
		$blogs = $this->model_mytheme_themeblog->getBlogs();

		foreach ($blogs as $blog) {
			$module_blog_data = array();

			$module = $this->model_mytheme_themeblog->getModule($blog['module_id']);

			$setting = json_decode($module['setting'],true);

			if (!empty($setting['status']) && $setting['status']) {
				$module_blogs = $this->model_mytheme_themeblog->getBlogsByModule($blog['module_id']);
			
			
				foreach ($module_blogs as $module_blog) {
				
				$total_comments = $this->model_mytheme_themeblog->getTotalBlogComments($module_blog['themeblog_id']);
					...
					$module_blog_data[] = array(
						...
						'href'        => $this->url->link('information/themeblog', 'themeblog_id=' . $module_blog['themeblog_id'], true)
					);
				}
		...
		$this->response->setOutput($this->load->view('information/themeblog_blogs', $data));
	}
}